"""
Debug / data-quality script — NOT part of the pipeline.

Simulates the Twitter bronze → silver transformation locally and reports
how many records are dropped at each step. Also checks for duplicates in
the processed S3 layer.

Usage:
    ENV=dev PYTHONPATH=. python scripts/debug_pipeline.py
"""
import gzip
import io
import json
...
def parse_twitter_ts(s: str | None) -> bool:
    """Returns True if the regex+format would produce a non-null timestamp."""
    if not s:
        return False
    m = TIMESTAMP_RE.match(s)
    if not m:
        return False
    return m.group(1) in MONTH_MAP
